...
\label{bsp:2.4}
	...
	\begin{proof}
		...
		\begin{align*}
			...
			&f \text{ invertierbar als stetige Funktion in einer Umgebung von } x \} = \mathcal{F}_x^*.
		\end{align*}
...
\begin{bsp}
	...
	\[
		f^{\#}_U\colon \mathcal{O}_Y \to f_*\mathcal{O}_X = \mathcal{O}_X(f^{-1}(U)),\; g \mapsto g \circ f.
	\]
\end{bsp}

\begin{bem}
\label{bem:2.9}
	Sei $(f,f^{\#})$ ein Mirphismus von geringten Räumen wie in Definition~\ref{defn:2.7}. Sei $x \in X$ und $y \coloneqq f(x)\in Y$. Dann haben wir einen kanonischen Homomorphismus
	\[
		f_x^{\#}\colon \mathcal{O}_{Y,y} \to \mathcal{O}_{Y,y},\; [(U,g)] \mapsto [(f^{-1}(U),f^{\#}(g))]
	\]
	von Ringen.
\end{bem}
